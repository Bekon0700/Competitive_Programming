...
int lx,lz;
char x[10002],z[102];
char an[10001][101][104];



void go(int cx,int cz)
{
    if(cz==lz)   {an[cx][cz][0]='1';return ;}
    if(cx==lx)  {an[cx][cz][0]='0';return ;}
    if(mm[cx][cz])  return ;
    mm[cx][cz] = true;
    ...
    if(x[cx]==z[cz])
    {
        ...
        for(c=i=0;i<103;i++)
        {
            c+=an[cx][cz][i]+an[cx+1][cz+1][i]-96;
            an[cx][cz][i] = c%10+48;
            c/=10;
        }
