...
int go(int l,int a,int b)
{
    ...
    for(i=0;i<l;i++)
        ...
        {
            if(i==0 && a==j)    continue;
            if(i==l-1 && b==j)    continue;
            k = go(i,a,j)^go(l-i-1,j,b);
            st.insert(k);
        }


    for(i=0;;i++)
        if(st.find(i)==st.end())    return re=i;
}
        ...
        for(i=0;i<L;i++)
            if(ar[i]!='.')
            {
                ...
                if(i&&ar[i-1]=='.')
                {
                    ...
                    k^=go(i-l-1,fun(l),fun(i));
                    //printf("%d %d %d %d\n",i-l-1,fun(l),fun(i),go(i-l-1,fun(l),fun(i)));
                }
                l=i;
            }
