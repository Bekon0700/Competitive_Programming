...
bool Find(int s,int t)
{
    ...
    while(!q.empty())
    {
        u=q.front();q.pop();
        for(i=0;i<SZ(g[u]);i++)
            if(par[g[u][i]]==-1 && mat[u][g[u][i]]>0)
            {
                par[g[u][i] ]=u;
                if(g[u][i]==t)  return true;
                q.push(g[u][i]);
            }
    }
    return false;
...
int main()
{
    ...
    int i,j,test,Case=1,n,m,c,u,v,re;
    ...
    while(scanf("%d%d",&n,&m)==2 &&(n||m))
    {
        ...
        AddEdge(1,n+1,two(30),0);
        AddEdge(n,n+n,two(30),0);
