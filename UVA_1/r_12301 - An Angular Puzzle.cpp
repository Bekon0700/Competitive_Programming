...
double fb(double a,double A,double B){
    return a*sin(B*pi/180.0)/sin(A*pi/180.0);
}

double Find(double a,double b,double C)
{
    double t,h,base,area;
    C*=pi/180.0;
    area = .5*a*b*sin(C);
    h = 2*area/b;
    base=b-a*cos(C);
    return atan2(h,base)*180.0/pi;
}
...
int main()
{
    ...
    while(scanf("%d%d%d%d%d",&b[0],&b[1],&b[2],&b[3],&b[4])==5&&b[0])
    {
        j=0;
        for(i=0;i<5;i++)
        {
        ...
        an = Find( fb(10,t[7],t[2]+t[3]) - fb(10,t[6],t[3]),
                   fb(10,t[9],t[4]+t[5]) - fb(10,t[6],t[5]), t[6]);
